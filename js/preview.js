...
(function (root, factory) {
  ...
  function wbToHtml(wb) {
    ...
    var colWs = [], colMaxChars = [];
    for (var c = 1; c <= maxCol; c++) {
      var col = ws.getColumn(c);
      colWs[c] = (col && col.width) ? Math.round(col.width * 7) : 0;
      colMaxChars[c] = 0;
    }
    // v1.5.26 自适应列宽：未显式设置 width 的列，按该列实际内容最大字符数推算宽度，避免被挤竖排
    ws.eachRow({ includeEmpty: false }, function (row, rn) {
      row.eachCell({ includeEmpty: false }, function (cell, cn) {
        if (cn > maxCol) return;
        var txt = (cellText(cell.value) || '').toString();
        var maxLen = 0;
        txt.split('\n').forEach(function (line) { if (line.length > maxLen) maxLen = line.length; });
        if (maxLen > colMaxChars[cn]) colMaxChars[cn] = maxLen;
      });
    });
    // 合并格：左上格内容字符数应用到所有被覆盖列（否则横跨多列的标签"我司全称"只计到首列，其他列仍 50px 太窄）
    merges.forEach(function (m) {
      var row = ws.getRow(m.top + 1);
      var cell = row && row.getCell(m.left + 1);
      if (!cell) return;
      var txt = (cellText(cell.value) || '').toString();
      var maxLen = 0;
      txt.split('\n').forEach(function (line) { if (line.length > maxLen) maxLen = line.length; });
      for (var cc = m.left; cc <= m.right; cc++) {
        if (cc + 1 <= maxCol && maxLen > colMaxChars[cc + 1]) colMaxChars[cc + 1] = maxLen;
      }
    });
    for (var c = 1; c <= maxCol; c++) {
      if (!colWs[c]) {
        // 粗略像素估算：每字符 ≈ 10px（中英文混排安全值）；最小 50px，最大 320px
        var px = Math.min(320, Math.max(50, Math.round(colMaxChars[c] * 10)));
        colWs[c] = px;
      }
    }
    ...
    try {
      ...
      for (var ig = 0; ig < imgs.length; ig++) {
        ...
        var w, h;
        if (r1) {
          // twoCellAnchor：用 br 算尺寸（旧 logo 写法）
          var x1 = colX(r1.nativeCol + 1); var y1 = rowY(r1.nativeRow + 1);
          w = x1 - x0; h = y1 - y0;
        } else if (im.range.ext) {
          // oneCellAnchor（产品图：借鉴 FBA v1 用 tl+ext）：尺寸来自 ext(cx,cy EMU) -> px
          w = (im.range.ext.cx || 0) / 9525; h = (im.range.ext.cy || 0) / 9525;
          if (!w || !h) { w = 92; h = 92; }
        } else { w = 92; h = 92; }
        var extra = '';
