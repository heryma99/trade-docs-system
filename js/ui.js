...
(function () {
  ...
  async function renderWizStepInner() {
    ...
    if (w.step === 1) {
      ...
      // v1.6.10: 智能装箱单渲染 —— 勾选多订单时，若关联了多个 packing 则自动合并成 1 行显示
      var _relatedPks = [];
      for (var _ri = 0; _ri < pks.length; _ri++) {
        var _inter = (pks[_ri].orderNos || []).filter(function (n) { return selNos.indexOf(n) >= 0; });
      ...
      if (_relatedPks.length > 1) {
        ...
        _relatedPks.forEach(function (_rp) {
          (_rp.boxes || []).forEach(function (b) { _mBoxes.push(b); });
          (_rp.orderNos || []).forEach(function (n) { _mOrderNos[n] = 1; });
          _mBoxCount += (_rp.totals && _rp.totals.boxCount) || 0;
          _mQty += (_rp.totals && _rp.totals.qty) || 0;
          _mFileNames.push(_rp.fileName || _rp.id);
        });
        var _mergedId = 'merged-' + _relatedPks.map(function (p) { return p.id; }).join('_');
        // 自动预选合并行
        if (!w.packingId || w.packingId.indexOf('merged-') !== 0) w.packingId = _mergedId;
        pkRows = '<tr class="checkrow" style="background:#f0fdf4;border-left:3px solid #16a34a"><td><input type="radio" name="wz-pk" value="' + _mergedId + '" checked></td>' +
          '<td><b>📦 合并装箱清单（' + _relatedPks.length + '份）</b><br><small style="color:#666">' + _mFileNames.join(' / ') + '</small></td>' +
          '<td class="mono">' + esc(Object.keys(_mOrderNos).join(', ')) + '</td>' +
          '<td class="num"><b>' + _mBoxCount + '</b></td><td class="num"><b>' + _mQty + '</b></td>' +
      ...
      document.getElementById('wz-next1').onclick = async function () {
        ...
        // v1.6.9: 允许不手选 radio——若勾选订单已关联装箱清单则自动合并；仅当既没手选也无线索时才拦截

        var _autoPids = [];

        for (var _i = 0; _i < w.orderIds.length; _i++) { var _o = await db.get('orders', w.orderIds[_i]); if (_o && _o.packingId && _autoPids.indexOf(_o.packingId) < 0) _autoPids.push(_o.packingId); }

        if (!pk && _autoPids.length === 0) { toast('请选择装箱清单，或确保勾选的订单已关联装箱清单', 'err'); return; }
    ...
    if (w.step === 4) {
      ...
      // v1.5.52 取图失败清单：引擎已对每条 SKU 走完「同域→GitHub Pages→jsDelivr CDN→Statically CDN→raw→CloudStudio」全候选链并各重试,



      // 仍失败说明整条 GitHub 分发网络在当前环境都不可达, 明确列出(不再静默空白)。
      ...
      if (ed && ed.failed && ed.failed.length) {
        ...
        embWarn += '<div class="vres warn">⚠️ 以下 ' + ed.failed.length + ' 个 SKU 经「同域→GitHub Pages→jsDelivr→raw→CloudStudio」全候选链多次重试后仍取图失败（当前网络到 GitHub 分发网络不可达/超时），导出后对应单元格为空白：<span class="mono">' + flist + '</span>。若整批失败，请检查网络后硬刷(Ctrl+Shift+R)重试，或在网络通畅的环境打开本页。</div>';
  ...
  // v1.6.9: 从勾选订单关联 + 手动选择两个来源收集 packing，多个则合并为 1 个 mergedPacking

  async function _resolveMergedPacking(w) {

    var pids = [];

    for (var i = 0; i < w.orderIds.length; i++) {

      var o = await db.get('orders', w.orderIds[i]);

      if (o && o.packingId && pids.indexOf(o.packingId) < 0) pids.push(o.packingId);

    }

    if (w.packingId && pids.indexOf(w.packingId) < 0) pids.push(w.packingId);
    ...
    for (var j = 0; j < pids.length; j++) {
      ...
      var p = await db.get('packings', pids[j]);
    ...
    return _mergePackings(pks);

  }



  // v1.6.9: 把多个 packing 的 boxes 拼接、orderNos/totals 累加，合成 1 个 packing 对象（引擎只收 1 个 packing，故无需改引擎）

  function _mergePackings(list) {

    var boxes = [], orderNos = {}, boxCount = 0, qty = 0;

    list.forEach(function (p) {

      (p.boxes || []).forEach(function (b) { boxes.push(b); });

      (p.orderNos || []).forEach(function (n) { orderNos[n] = 1; });

      boxCount += (p.totals && p.totals.boxCount) || 0;

      qty += (p.totals && p.totals.qty) || 0;

    });

    return {

      id: 'merged-' + list.map(function (p) { return p.id; }).join('_'),

      fileName: '合并装箱清单（' + list.length + '份）',

      boxes: boxes,

      orderNos: Object.keys(orderNos),

      totals: { boxCount: boxCount, qty: qty }
  ...
  async function buildWizContext(w, kind) {
    ...
    // v1.6.9: 自动合并多装箱清单——从勾选订单各自关联的 packing 收集并合并（支持多个独立装箱单合并成一张发票）

    var packing = await _resolveMergedPacking(w);
